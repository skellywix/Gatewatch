...
Gatewatch has two authentication modes:
...
- `trusted_proxy`: on-prem production target. A reverse proxy performs AD SSO and injects trusted identity headers. The app ignores browser-supplied role and actor headers, derives the actor from proxy identity, maps the configured Admin group to the Admin role, and assigns every other authenticated user to the User role.
...
- Authentication mapping settings for the intended AD or Entra Admin group.
...
| --- | --- | --- |
| Admin | `create`, `update`, `review`, `import` | Full administration, backend setup, imports, AD sync, backups, auth settings, connector setup, email provider settings, and audit export. |
| User | `create`, `update`, `review`, `import` | Daily access inventory work: employees, resources, access records, requests, reviews, removals, imports, shared accounts, physical credentials, and email action routing. |

The route allowlist narrows these verbs. User has the normal workflow verbs, but cannot run backend setup routes that can break the app or expose sensitive operating data.

## Visible Workspaces

The frontend keeps the daily workflow separated by account access level. Admin users see every workspace. User accounts see the primary workspaces and the advanced operational workspaces needed for daily tracking. Direct browser navigation to a hidden workspace is redirected to the first allowed view. This UI filtering is a usability guard only; server-side authorization below remains the source of truth.

| Role | Visible workflow |
| --- | --- |
| Admin | Full app, including Settings, AD Sync, connector plans, backups, auth settings, email provider settings, and audit export. |
| User | Home, People, Access, Tasks, Access Requests, Catalog, Reviews, Risk Center, Offboarding, Assets, Imports, and Audit Log without backend setup controls. |
...
In `trusted_proxy` mode, every static and API request must include a proxy-authenticated user header. A user in the configured Admin group becomes Admin. Any other authenticated user becomes User. Client-supplied app role and actor headers are ignored in this mode.

Scheduled AD payloads and backup filesystem paths are Admin-only. User bootstrap payloads omit AD sync settings, backup runs, connector plans, and full auth settings. User requests to backend setup endpoints return HTTP 403.
...
| --- | --- | --- |
| Employees | `POST /api/employees`, `PATCH /api/employees/{id}` | Admin, User |
| Manual override fields | `admin_override`, `admin_notes` on employee PATCH | Admin only |
| Business categories | `POST /api/resource-categories` | Admin, User |
| Systems, locations, and resources | `POST /api/systems` | Admin, User |
| Direct access records | `POST /api/access-records` | Admin, User |
| Access review decisions | `POST/PATCH /api/access-records/{id}/review` | Admin, User |
| Removal evidence and access updates | `PATCH /api/access-records/{id}` | Admin, User |
| Account CSV import | `POST /api/imports/accounts` | Admin, User |
| AD sync and scheduled AD sync | `POST /api/ad/sync`, `POST /api/ad-sync-settings`, `POST /api/ad/run-scheduled` | Admin |
| Access requests | `POST /api/access-requests` | Admin, User |
| Request decisions | `POST /api/access-requests/{id}/decision` | Admin, User |
| Email provider settings | `POST /api/email-settings` | Admin |
| Email action routes | `POST /api/access-requests/{id}/email-route`, `PATCH /api/email-routes/{id}` | Admin, User |
| Review campaigns | `POST/PATCH /api/review-campaigns` | Admin, User |
| Notifications | `PATCH /api/notifications/{id}` | Admin, User |
| Shared accounts | `POST /api/shared-accounts` | Admin, User |
| Physical credentials | `POST /api/physical-credentials` | Admin, User |
| Connector plans | `POST /api/connectors` | Admin |
| Backups | `GET /api/backups`, `POST /api/backups/run` | Admin |
| Auth settings | `GET /api/auth-settings`, `POST /api/auth-settings` | Admin |
| Audit CSV export | `GET /api/audit-log.csv` | Admin |
| Disabled-user removal routing | `POST /api/disabled-access/route-removal` | Admin, User |
...
- A route-to-remove action sets access to `removal_pending` and gives a default three-day removal due date.
- Access request approval creates an access record only after an Admin or User decision.
...
- Backups are logged in `backup_runs`, use collision-resistant filenames, require retention between 1 and 3650 days, hide filesystem paths from non-admin payloads, and mark expired managed backup files with `pruned_at` after successful backup runs.
...
- Derive Admin from AD or Entra group membership.
